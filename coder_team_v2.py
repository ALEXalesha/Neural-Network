...
# Патч сломанного simplejson (мешает импорту requests)
try:
...
def is_approved(text: str) -> bool:
    ...
    # Явное одобрение где угодно в тексте (включая print("ALL TESTS PASS") в коде)
    STRONG_APPROVE = (
        'all tests pass', 'approved', 'lgtm', 'no issues found',
        'одобрен', 'все тесты проходят', 'замечаний нет',
    )
    for kw in STRONG_APPROVE:
        if kw in lower:
            return True
...
def _extract_defs_only(code: str) -> str:
    ...
    DANGEROUS = ('while ', 'while\t', 'if __name__')
...
def agent_tester(task: str, code: str) -> tuple[str, bool]:
    ...
    try:
        ...
        # Auto-install pytest если нужен
        if 'import pytest' in test_code or 'from pytest' in test_code:
...
# ──────────────────────────────────────────────────────
CONTROL_FILE = BASE / "coder_v2_control.json"
...
def run_pipeline(task: str, save: bool = True) -> str:
    print(f"\n{'═'*60}")
    print(f"  CODER TEAM v2 — АВТОНОМНЫЙ ЦИКЛ")
    print(f"  Задача: {task}")
    ...
    for iteration in range(1, MAX_ITERATIONS + 1):
        ...
        print(f"\n{'━'*60}")
        print(f"  ПРОВЕРКА (ревьюер + тестировщик)")
        print(f"{'━'*60}", flush=True)
        ...
        if any(v is None for v in [review_ok, test_ok]):
            print(f"  (Агент пропущен — ошибка LM Studio, его вето не учитывается)")
        print(f"{'━'*60}", flush=True)
    ...
    if save:
        out_file = BASE / "coder_output_v2.py"
        out_file.write_text(
...
def run_review(code: str, task_desc: str = "", save: bool = True) -> str:
    ...
    print(f"\n{'═'*60}")
    print(f"  CODER TEAM v2 — ПРОВЕРКА КОДА")
    print(f"  LM Studio: {LM_URL}")
    print(f"  Макс. итераций: {MAX_ITERATIONS}")
    print(f"{'═'*60}")
