...
#   bash tools/publish_github.sh v1.4.0     # код и тег
#
# Источник правды - Gitea. На GitHub уезжает копия main, в которой личный gmail автора
# и коммитера заменён на анонимную почту аккаунта GitHub: почта автора видна любому, кто
# откроет коммит. Рабочая история не трогается. Ветка переписывается каждый раз заново,
# поэтому пуш --force: в main на GitHub никто, кроме этого скрипта, не пишет.
#
# Веса в LFS уезжают тем же пушем: pre-push хук git-lfs отправляет объекты, на которые
# ссылаются публикуемые коммиты. Замена почты указатели LFS не трогает.
set -euo pipefail
